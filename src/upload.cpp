...
namespace mediawiki
{
class UploadPrivate : public WorkPrivate
{

public:

    UploadPrivate(WPMediaWiki& mediawiki)
...
Upload::Upload(WPMediaWiki& mediawiki, QObject* const parent)
    : Work(*new UploadPrivate(mediawiki), parent)
{
}
...
void Upload::start()
{
    ...
    Query* info = new Query(d->m_MediaWiki, this);
    info->setPageName(QStringLiteral("File:") + d->filename);
    info->setToken(QStringLiteral("edit"));

    connect(info, SIGNAL(page(WikiPage)),
            this, SLOT(doWorkSendRequest(WikiPage)));

    info->start();
}

void Upload::doWorkSendRequest(WikiPage page)
{
    Q_D(Upload);
    ...
    d->reply = d->mManager->post( request, out );
    connectReply();
